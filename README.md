...
- 논문에서는 2마디 기준 512 batch, 50000steps 학습함.
- 해당 레포에서는 256 batch, 360steps(20epochs) 학습.
- 학습이 진행될수록 loss는 지속적으로 감소함.
- Reconstruct accuracy 개선 정도가 적은 것을 보아 ELBO loss의 KL beta값을 줄이는 시도를 해보면 좋을 듯.
- Eval 점수가 지속적으로 향상되는 것을 보아 학습을 더 많이하는 경우 모델이 개선될 여지가 많아보임.
- 논문에서 사용한 scheduler, kl-annealing를 사용했지만, 더 적은 step에 적절한 공식을 적용하면 더 좋은 학습이 가능.
- Batch마다 accuracy 차이가 많이 나는 것으로 보아 random sampling을 적용하는 것이 좋아보임.

**모든 score는 마지막 bacth 기준**
